...
//    生成密钥保存文件
        Utils.writeFile(new File(dir, "password.ini"), password + ";" + altas, "utf-8");

        String cmd = String.format("keytool -genkey -noprompt -alias key -keyalg RSA -dname \"CN=DL, OU=DL, O=DL, L=City, S=Province, C=CN\" -keystore \"%s\" -storepass  %s -keypass %s -validity 1000000 -keysize 1024", keystoreFilePath, password, password);
