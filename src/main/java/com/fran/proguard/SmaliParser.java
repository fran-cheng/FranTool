...
public class SmaliParser {
	...
	private int mLine;
	private SmaliParser.Callback mCallback;

	public SmaliParser(String source, SmaliParser.Callback callback) {
		this.mSource = source;
		...
		while (true) {
			...
			if (!isIdentityChar(ch)) {
				if (!Character.isWhitespace(ch) && ch != -1) {
					throw new SmaliParser.ParseException("未识别标识符：" + (char) ch + ",行 " + this.mLine + ",列 " + this.mCol);
				} else {
					return this.mSource.substring(start, this.mIdx + 1);
				}
			}
		...
		while (true) {
			while (true) {
				...
				if (ch == 92) {
					...
					if (ch == -1) {
						throw new SmaliParser.ParseException("无效字符串格式：" + (char) ch + ",行 " + this.mLine + ",列 " + this.mCol);
					}
	...
	public static void main(String[] args) {
		...
		SmaliParser sl = new SmaliParser(s, new SmaliParser.Callback());
		String r = null;

		try {
			r = sl.parse();
		} catch (SmaliParser.ParseException var6) {
